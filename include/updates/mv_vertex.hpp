...
struct mv_tau_update {
    ...
    double attempt(){
        ...
        tau_proposed = tau_prev - std::log(1 - std_unif(*rng)*(std::exp(-deltaE*(tau_next - tau_prev))))/deltaE;
        ...
        const double numerator {std::exp(-deltaE * vertex->tau) * diagram_weight_proposed};
